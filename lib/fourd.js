...
var FourD = function(){
  ...
  */
  ...
  var TextVertex = Vertex.prototype;
  TextVertex.prototype.paint = function(){ 
    if(!this.options.cube.text){
      this.options.cube.text = '';
    }
    this.object = new Text(scene, this.options.cube);
  }
  ...
  */ 
  ...
  var TextCube = function(){
    ...
    if(options.text !== undefined){
      // from: https://stackoverflow.com/questions/12328714/convert-text-to-image-using-javascript
      var tCtx = document.createElement('canvas').getContext('2d'); //Hidden canvas
      var imageElem = document.createElementById(); //Image element
      // Text input element
      tCtx.canvas.width = tCtx.measureText(text).width;
      tCtx.fillText(text, 0, 10);
      // from: https://threejs.org/docs/#api/textures/Texture
      // load a texture, set wrap mode to repeat
      var texture = new THREE.TextureLoader().load( tCtx.canvas.toDataURL() );
